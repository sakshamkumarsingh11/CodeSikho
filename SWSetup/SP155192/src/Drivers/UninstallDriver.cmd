...
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_B520*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_B851*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_C852*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_B852*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_8852*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_A85A*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_A852*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_D723*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_C822*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_C821*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_C82B*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_8813*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_B822*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_B814*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_B821*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_8812*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_8821*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_818B*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_818C*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_8753*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_B723*" >> %logfile%
"%var%\rtdevcon.exe" remove "@PCI\VEN_10EC&DEV_8179*" >> %logfile%

"%var%\dpinst.exe" /Q /D /U "%var%\netrtwlane613.inf" >> %logfile%
"%var%\dpinst.exe" /Q /D /U "%var%\netrtwlane602.inf" >> %logfile%
"%var%\dpinst.exe" /Q /D /U "%var%\netrtwlane601.inf" >> %logfile%
"%var%\dpinst.exe" /Q /D /U "%var%\netrtwlane6.inf" >> %logfile%
"%var%\dpinst.exe" /Q /D /U "%var%\netrtwlane.inf" >> %logfile%
"%var%\dpinst.exe" /Q /D /U "%var%\netrtwlane02.inf" >> %logfile%

REM ---> Delete all netrtwlane613.inf
chcp 65001
...
FOR /f "tokens=2,3,4 delims=: " %%I IN ('pnputil /enum-drivers') do (
	...
	set OriginalName=%%J
	if "!OriginalName!"=="netrtwlane613.inf" (
...
REM ---> Delete all netrtwlane613.inf Finish

REM ---> Delete all netrtwlane602.inf
chcp 65001
set GetVersion=Disable
set deleteDriver=Disable
set RTPublishedName=NULL
FOR /f "tokens=2,3,4 delims=: " %%I IN ('pnputil /enum-drivers') do (
	set PublishedName=!OriginalName!
	set OriginalName=%%J
	if "!OriginalName!"=="netrtwlane602.inf" (
		call set RTPublishedName=!PublishedName!
		echo ---^> >> %logfile%
		echo !PublishedName! >> %logfile%
		call set GetVersion=Enable
...
REM ---> Delete all netrtwlane602.inf Finish

REM ---> Delete all netrtwlane601.inf
chcp 65001
set GetVersion=Disable
set deleteDriver=Disable
set RTPublishedName=NULL
FOR /f "tokens=2,3,4 delims=: " %%I IN ('pnputil /enum-drivers') do (
	set PublishedName=!OriginalName!
	set OriginalName=%%J
	if "!OriginalName!"=="netrtwlane601.inf" (
		call set RTPublishedName=!PublishedName!
		echo ---^> >> %logfile%
		echo !PublishedName! >> %logfile%
		call set GetVersion=Enable
...
REM ---> Delete all netrtwlane601.inf Finish
...
REM ---> Delete all netrtwlane6.inf Finish
...
"%var%\rtdevcon.exe" rescan >> %logfile%

set TARGET_SERVICE=rtwlane601
set SERVICE_STATE=
